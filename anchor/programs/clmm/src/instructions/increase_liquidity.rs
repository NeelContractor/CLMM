...
use crate::{states::{Pool, Position, TickArray}, utils::{ErrorCode, get_amounts_for_liquidity, get_sqrt_price_from_tick}};
...
pub fn increase_liquidity(ctx: Context<IncreaseLiquidity>, liquidity_amount: u128, lower_tick: i32, upper_tick: i32) -> Result<(u64, u64)> {
    let pool = &mut ctx.accounts.pool;
    let position = &mut ctx.accounts.position;

    require!(lower_tick < upper_tick && lower_tick % pool.tick_spacing == 0 && upper_tick % pool.tick_spacing == 0, ErrorCode::InvalidTickRange);
